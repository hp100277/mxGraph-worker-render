...
const mxConstants = require("./mxConstants");
const mxUtils = require("./mxUtils");
const mxAbstractCanvas2D = require("./mxAbstractCanvas2D");
...
mxVmlCanvas2D.prototype.addNode = function (filled, stroked) {
  ...
  if (node != null) {
    ...
    node.strokeweight =
      this.format(Math.max(1, (s.strokeWidth * s.scale) / this.vmlScale)) +
      "px";
...
mxVmlCanvas2D.prototype.createDiv = function (str, align, valign, overflow) {
  ...
  if (state.fontBackgroundColor != null) {
    css +=
      "background-color:" +
      mxUtils.htmlEntities(state.fontBackgroundColor) +
      ";";
  }

  if (state.fontBorderColor != null) {
    css +=
      "border:1px solid " + mxUtils.htmlEntities(state.fontBorderColor) + ";";
  }
